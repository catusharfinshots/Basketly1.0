...
import { LineChart, Twitter, Linkedin, Instagram, Youtube } from 'lucide-react';
...
const cols = [
  {
    ...
    links: [
      ...
      { name: 'Careers', to: '/business' },
      { name: 'Press', to: '/business' },
      { name: 'Contact', to: '/business' },
    ],
  },
  {
    title: 'Resources',
    links: [
      ...
      { name: 'Help centre', to: '/business' },
    ],
  },
  {
    title: 'Offerings',
    links: [
      { name: 'Model Portfolios', to: '/model-portfolios' },
      { name: 'AIF', to: '/aif' },
  ...
  },
  {
    title: 'Fine Print',
    links: [
      { name: 'Terms', to: '/business' },
      { name: 'Privacy', to: '/business' },
      { name: 'Disclosures', to: '/business' },
      { name: 'Investment tools', to: '/calculators' },
    ],
  },
];
  ...
  return (
    <footer className="mt-24 border-t border-[#E6E8F0] bg-[#F5F7FB]">
      <div className="container-x py-14">
          ...
          {cols.map((col) => (
            <div key={col.title}>
              ...
              <ul className="space-y-2">
                {col.links.map((l) => (
                  <li key={l.name}><Link to={l.to} className="text-sm text-[#64748B] hover:text-[#0F1729]">{l.name}</Link></li>
                ))}
              </ul>
            </div>
          ))}
        ...
        <div className="mt-12 pt-8 border-t border-[#E6E8F0] flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <Link to="/partner" data-testid="become-partner-link" className="text-xs font-semibold text-[#6C2BD9] hover:text-[#5320A8]">Become a partner →</Link>
        </div>
      </div>
    </footer>
  );
