...
- **Admin Database viewer (Jun 2026)**: read-only, admin-only "Database" tab in `/admin`. Backend `dbadmin.py` (routes `/api/admin/db/collections` and `/api/admin/db/{collection}` with skip/limit/q); allowlisted collections (users, leads, analyst_portfolios, analyst_invites, content, status_checks); recursively redacts sensitive fields (password/hash/secret/token). Frontend: collection pills w/ counts, search, expandable JSON records, pagination. Non-admins get 403. Works in production after redeploy (preview & prod DBs are separate).
